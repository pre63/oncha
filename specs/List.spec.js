...
describe('A List', () => {
  ...
  it('should be [1,2,3] when inspecting', () => assert.equal(list.inspect(), 'List([1,2,3])'))
  ...
  it('should include 2', () => assert.equal(List.of([1, 2, 3]).includes(2), true))

  it('should return 1 as an indexOf 2', () => assert.equal(List.of([1, 2, 3]).indexOf(2), 1))

  it('should the last index of ', () => assert.equal(List.of([1, 2, 3, 3, 3, 3]).lastIndexOf(3), 5))
  ...
  it('should have a lenght of 4', () => assert.equal(list.concat([5], [6, 7]).length(), 6))

  it('shoud reduce arrays to a single value', () =>
    Id(List([1, 2, 3]).reduce(acc => x => acc + x)(0)).map(a => a === 6).map(assert))
  ...
  it('shoud reduce arrays to a single array when empty', () =>
    Id(List([[], [], []]).reduce(acc => x => acc.concat(x))([])).map(a => assert.deepEqual(a, [])))

  it('shoud reduceRight arrays to a single value', () =>
    Id(List([1, 2, 3]).reduceRight(acc => x => acc + x)(0)).map(a => a === 6).map(assert))

  it('shoud reduceRight arrays to a single array', () =>
    Id(List([[1], [2], [3]]).reduceRight(acc => x => acc.concat(x))([])).map(a =>
  ...
  describe('Fold', () => {
    ...
    it('Should fold with a function', () =>
      Id(List([1, 2, 3, 4]).fold(a => a)).map(a => assert.deepEqual(a, [1, 2, 3, 4])))

    it('Should fold with a null function', () =>
      Id(List([1, 2, 3, 4]).fold(null)).map(a => assert.deepEqual(a, [1, 2, 3, 4])))
